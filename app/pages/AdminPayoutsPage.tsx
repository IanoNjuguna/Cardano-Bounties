...
export function AdminPayoutsPage() {
  ...
  const items = useMemo(() => {
    let list = data?.queues.approved_payouts || [];
  ...
  const runManualPayout = async () => {
    ...
    try {
      const response = await authFetch("/api/admin/release-payment", {
        method: "POST",
        body: JSON.stringify({ submission_id: selectedItem.id, transaction_hash: txHash }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || "Action failed.");
      
      toast.success("Payout Recorded", "Payout transaction recorded successfully.");
      
      setData((prev) => {
        if (!prev) return prev;
        const updatedQueue = prev.queues.approved_payouts?.filter(s => s.id !== selectedItem.id);
        return { ...prev, queues: { ...prev.queues, approved_payouts: updatedQueue } };
      });
  ...
  const handleExecuteOnChainPayout = async () => {
    if (!selectedItem) return;
    if (!wallet || !address) {
      toast.error("Wallet Required", "Please connect a wallet to execute on-chain payouts.");
      return;
    }

    const bounty = getSubmissionBounty(selectedItem);
    const contributorAddress = selectedItem.contributor?.stake_address;

    if (!bounty || !bounty.escrow_tx_hash) {
      toast.error("Data Missing", "Bounty escrow transaction hash is missing. Cannot proceed.");
      return;
    }

    if (!contributorAddress) {
      toast.error("Data Missing", "Contributor address is missing. Cannot proceed.");
      return;
    }
    ...
    try {
      toast.success("Building Transaction", "Preparing payout transaction...");

      const newTxHash = await releaseBountyPayout({
        wallet,
        recipientAddress: contributorAddress,
        lovelace: Math.floor(Number(bounty.reward_amount || 0) * 1000000),
      });

      toast.success("Transaction Submitted", `Payout Tx Hash: ${shortId(newTxHash)}`);
      
      // Auto-record the payout
      const response = await authFetch("/api/admin/release-payment", {
        method: "POST",
        body: JSON.stringify({ submission_id: selectedItem.id, transaction_hash: newTxHash }),
      });
      
      if (!response.ok) {
         toast.error("Warning", "Transaction was submitted on-chain but backend recording failed. Please manually record it.");
         setTxHash(newTxHash);
      } else {
         setData((prev) => {
          if (!prev) return prev;
    ...
    <div className={styles.container}>
      <div className={styles.controls}>
        ...
        <div className={styles.search}>
          ...
          <input
            type="text"
            placeholder="Search contributor or bounty..."
            value={search}
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table} role="grid" aria-label="Queued Payouts">
          <thead>
            <tr>
              <th data-sortable="true" onClick={() => handleSort("submitter")} aria-sort={sortCol === "submitter" ? (sortDesc ? "descending" : "ascending") : "none"}>
                <div className={styles.thContent}>Contributor {renderSortIndicator("submitter")}</div>
              </th>
              <th>Bounty</th>
                  ...
                  <div className={styles.emptyState}>
                    ...
                    {search || filter !== "all" ? (
                      ...
                      <>
                        ...
                        <p>There are no approved submissions waiting for payout.</p>
                      </>
                    )}
                  </div>
                </td>
                  ...
                  <tr key={submission.id} onClick={() => handleRowClick(submission.id)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleRowClick(submission.id); } }}>
                    ...
                    <td>
                      <div className={styles.actions}>
                        <button type="button" aria-label="View payout details" tabIndex={-1} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'inherit' }}>
          ...
          <div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="modal-title">
            ...
            <div className={styles.modalBody}>
              ...
              <div className={styles.contentSection}>
                <div className={styles.contentBlock}>
                  <div className={styles.contentLabel}>Recipient Address</div>
                  <div className={styles.contentValue} style={{ fontFamily: 'ui-monospace, monospace', fontSize: '12px', wordBreak: 'break-all' }}>
                    {selectedItem.contributor?.stake_address || "Address not recorded"}
                  </div>
                </div>
              </div>
            ...
            <div className={styles.modalFooter}>
              <div className={styles.navControls}>
                <button type="button" className={styles.navBtn} disabled={!canGoPrev} aria-label="Previous payout" onClick={() => setSelectedSubmissionId(items[selectedIndex - 1]?.id || null)}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
                </button>
                <button type="button" className={styles.navBtn} disabled={!canGoNext} aria-label="Next payout" onClick={() => setSelectedSubmissionId(items[selectedIndex + 1]?.id || null)}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                </button>
              </div>

              <button type="button" className={styles.rejectBtn} disabled={isSubmitting || isExecutingOnChain} onClick={() => void runManualPayout()}>
                {isSubmitting ? <div className={styles.spinner} /> : "Record Manual Payout"}
              </button>
              <button type="button" className={styles.approveBtn} disabled={isSubmitting || isExecutingOnChain} onClick={() => void handleExecuteOnChainPayout()}>
                {isExecutingOnChain ? <div className={styles.spinner} /> : "Release Payment On-Chain"}
              </button>
            </div>
          </div>
        </div>
